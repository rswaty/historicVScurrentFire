...
## Fire across time-a dramatization

Looking at historical amounts of fire can help put today's fires in ecological context.  In the animation below we took the modeled historical values and assumed a little bit of variation until ~1920 when we assumed fire suppression started.  The values from 1800-1999 are purely assumed with no modeling or real data.  We do believe the trends indicated are realistic and highlight the dramatic change in fire regimes across the Huron-Manistee National Forest

<br>


```{r animatedPlotly, echo=FALSE, message=FALSE, warning=FALSE, fig.align='left', fig.width=12, fig.height=7}
# insert choroMap.html URL
htmltools::tags$iframe(title = "My embedded document", src = "fireDrama.html", frameborder = "1", width = "90%", height="700", vspace="30")
...
The combined data has information we do not need and is not formatted properly to explore historical fire regimes for our landscape.  

**Raw data available here:**

```{r hmnf raw, echo=FALSE, message=FALSE, warning=FALSE}
hmnf %>%
  download_this(
    output_name = "Raw combined data for Huron-Manistee NF",
...
We did some wrangling.  Hit the "Code" button at the top-right of the page to see the code.

<br>

```{r bpsWrangle2, echo=FALSE, message=FALSE, warning=FALSE}
hmnf_bps <- hmnf %>%
  select(3, 12:17) %>%
  group_by(BPS_NAME,
           FRI_REPLAC,
...
**Wrangled data available here:**

```{r hmnf wrangled, echo=FALSE, message=FALSE, warning=FALSE}
hmnf_bps %>%
  download_this(
    output_name = "Wrangled Huron-Manistee Historical Data",
    output_extension = ".csv",
    button_label = "Download data",
    button_type = "warning",
    has_icon = TRUE,
